...
#if defined(__cpp_nontype_template_parameter_class) && (!FMT_GCC_VERSION || FMT_GCC_VERSION >= 903)
...
class format_string_compiler : public error_handler {
 ...
 public:
  ...
  FMT_CONSTEXPR void on_text(const Char *begin, const Char *end) {
    if (begin != end) handler_(part::make_text({begin, to_unsigned(end - begin)}));
  }
...
struct field {
  ...
  constexpr OutputIt format(OutputIt out, const Args &... args) const {
    if
      constexpr(is_named_arg<typename std::remove_cv<T>::type>::value) {
...
struct runtime_named_field {
  ...
  constexpr static bool try_format_argument(OutputIt &out, basic_string_view<Char> arg_name, const T &arg) {
    if
      constexpr(is_named_arg<typename std::remove_cv<T>::type>::value) {
...
constexpr auto parse_tail(T head, S format_str) {
  if
    constexpr(POS != basic_string_view<typename S::char_type>(format_str).size()) {
      constexpr auto tail = compile_format_string<Args, POS, ID>(format_str);
      if
        constexpr(std::is_same<remove_cvref_t<decltype(tail)>, unknown_format>()) return tail;
      else
...
constexpr auto compile_format_string(S format_str) {
  ...
  if
    constexpr(str[POS] == '{') {
      if
        constexpr(POS + 1 == str.size()) throw format_error("unmatched '{' in format string");
      if
        constexpr(str[POS + 1] == '{') { return parse_tail<Args, POS + 2, ID>(make_text(str, POS, 1), format_str); }
      else if
        constexpr(str[POS + 1] == '}' || str[POS + 1] == ':') {
          ...
          if
            constexpr(str[POS + 1] == '}') {
              ...
              return parse_tail<Args, POS + 2, next_id>(field<char_type, id_type, ID>(), format_str);
            }
          else {
            constexpr auto result = parse_specs<id_type>(str, POS + 2, ID + 1);
            return parse_tail<Args, result.end, result.next_arg_id>(spec_field<char_type, id_type, ID>{result.fmt},
                                                                    format_str);
          }
        }
      else {
        ...
        if
          constexpr(arg_id_result.arg_id.kind == arg_id_kind::index) {
            ...
            if
              constexpr(c == '}') {
                return parse_tail<Args, arg_id_end_pos + 1, manual_indexing_id>(field<char_type, id_type, arg_index>(),
                                                                                format_str);
              }
            else if
              constexpr(c == ':') {
                constexpr auto result = parse_specs<id_type>(str, arg_id_end_pos + 1, 0);
                return parse_tail<Args, result.end, result.next_arg_id>(
                    spec_field<char_type, id_type, arg_index>{result.fmt}, format_str);
              }
          }
        else if
          constexpr(arg_id_result.arg_id.kind == arg_id_kind::name) {
            if
              constexpr(c == '}') {
    ...
    if
      constexpr(end - POS > 1) { return parse_tail<Args, end, ID>(make_text(str, POS, end - POS), format_str); }
    else {
...
constexpr auto compile(S format_str) {
  ...
  if
    constexpr(str.size() == 0) { return detail::make_text(str, 0, 0); }
  else {
    constexpr auto result = detail::compile_format_string<detail::type_list<Args...>, 0, 0>(format_str);
    if
      constexpr(std::is_same<remove_cvref_t<decltype(result)>, detail::unknown_format>()) {
...
template <typename OutputIt, typename CompiledFormat, typename... Args,
          FMT_ENABLE_IF(detail::is_compiled_format<CompiledFormat>::value)>
constexpr OutputIt format_to(OutputIt out, const CompiledFormat &cf, const Args &... args) {
...
#ifdef __cpp_if_constexpr
  if
    constexpr(std::is_same<typename S::char_type, char>::value) {
      constexpr basic_string_view<typename S::char_type> str = S();
      if
        constexpr(str.size() == 2 && str[0] == '{' && str[1] == '}') {
          const auto &first = detail::first(args...);
          if
            constexpr(detail::is_named_arg<remove_cvref_t<decltype(first)>>::value) {
...
constexpr detail::udl_compiled_string<remove_cvref_t<decltype(Str.data[0])>, sizeof(Str.data), Str> operator""_cf() {
  return {};
}
}  // namespace literals
#endif
