...
struct is_output_iterator<It, T, void_t<typename std::iterator_traits<It>::iterator_category,
...
constexpr const U &check(const U &val) {
  return val;
}
...
class basic_format_context {
 ...
 public:
  ...
  FMT_CONSTEXPR format_arg arg(basic_string_view<char_type> name) { return args_.get(name); }
...
constexpr format_arg_store<Context, Args...> make_format_args(const Args &... args) {
  return {args...};
}
...
class basic_format_args {
 ...
 private:
  ...
  constexpr bool is_packed() const { return (desc_ & detail::is_unpacked_bit) == 0; }
...
template <typename S, typename... Args, typename Char = char_t<S>,
          FMT_ENABLE_IF(!FMT_COMPILE_TIME_CHECKS || !std::is_same<Char, char>::value)>
