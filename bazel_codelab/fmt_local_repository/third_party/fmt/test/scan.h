...
class scan_parse_context {
 ...
 public:
  ...
  explicit FMT_CONSTEXPR scan_parse_context(string_view format) : format_(format) {}

  FMT_CONSTEXPR iterator begin() const { return format_.begin(); }
  FMT_CONSTEXPR iterator end() const { return format_.end(); }

  void advance_to(iterator it) { format_.remove_prefix(detail::to_unsigned(it - begin())); }
};
...
struct scan_context {
 ...
 public:
  ...
  void advance_to(iterator it) { input_.remove_prefix(detail::to_unsigned(it - begin())); }
};
...
class scan_arg {
 public:
  ...
  scan_arg(long long& value) : type(scan_type::long_long_type), long_long_value(&value) {}
  scan_arg(unsigned long long& value) : type(scan_type::ulong_long_type), ulong_long_value(&value) {}
  scan_arg(std::string& value) : type(scan_type::string_type), string(&value) {}
  scan_arg(fmt::string_view& value) : type(scan_type::string_view_type), string_view(&value) {}
  template <typename T>
 ...
 private:
  template <typename T>
  static void scan_custom_arg(void* arg, scan_parse_context& parse_ctx, scan_context& ctx) {
    scanner<T> s;
...
struct scan_args {
  ...
  template <size_t N>
  scan_args(const std::array<detail::scan_arg, N>& store) : size(N), data(store.data()) {
...
struct scan_handler : error_handler {
 ...
 public:
  ...
  void on_text(const char* begin, const char* end) {
    ...
    if (it + size > scan_ctx_.end() || !std::equal(begin, end, make_checked(it, size))) {
...
string_view::iterator vscan(string_view input, string_view format_str, scan_args args) {
...
template <typename... Args>
string_view::iterator scan(string_view input, string_view format_str, Args&... args) {
