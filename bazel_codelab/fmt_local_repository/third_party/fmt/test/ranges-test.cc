...
TEST(RangesTest, FormatTuple) {
  std::tuple<int64_t, float, std::string, char> t{42, 1.5f, "this is tuple", 'i'};
...
struct my_struct {
  ...
  template <size_t N>
  fmt::enable_if_t<N == 0, int32_t> get() const noexcept {
    return i;
  }
  template <size_t N>
...
template <>
struct tuple_size<my_struct> : std::integral_constant<size_t, 2> {};
...
TEST(RangesTest, PathLike) { EXPECT_FALSE((fmt::is_range<path_like, char>::value)); }
...
TEST(RangesTest, FormatStringLike) { EXPECT_EQ("foo", fmt::format("{}", string_like())); }
...
TEST(RangesTest, JoinInitializerList) {
  ...
  EXPECT_EQ("fmt rocks !", fmt::format("{}", fmt::join({"fmt", "rocks", "!"}, " ")));
}
...
TEST(RangesTest, JoinRange) {
  ...
  EXPECT_EQ("0,0,0", fmt::format("{}", fmt::join(noncopyable_range<int>(3u, 0), ",")));

  non_const_only_range<int> x(3u, 0);
  EXPECT_EQ("0,0,0", fmt::format("{}", fmt::join(x, ",")));
  EXPECT_EQ("0,0,0", fmt::format("{}", fmt::join(non_const_only_range<int>(3u, 0), ",")));

  std::vector<int> y(3u, 0);
  EXPECT_EQ("0,0,0", fmt::format("{}", fmt::join(y, ",")));
  EXPECT_EQ("0,0,0", fmt::format("{}", fmt::join(std::vector<int>(3u, 0), ",")));
