...
struct std::formatter<color> : std::formatter<const char*> {
  auto format(color c, format_context& ctx) { return formatter<const char*>::format(color_names[c], ctx); }
};
...
struct std::formatter<S> {
  ...
  constexpr auto parse(format_parse_context& ctx) {
    constexpr auto is_ascii_digit = [](const char c) { return c >= '0' && c <= '9'; };
    ...
    ++iter;
    char c = get_char();
    if (!is_ascii_digit(c) || (++iter, get_char()) != '}') throw format_error("invalid format");
  ...
  auto format(S s, format_context& ctx) {
    int width = visit_format_arg(
        [](auto value) -> int {
          ...
          if
            constexpr(!is_integral_v<type> || is_same_v<type, bool>) throw format_error("width is not integral");
          // else if (value < 0 || value > numeric_limits<int>::max())
          else if (fmt::detail::is_negative(value) || value > numeric_limits<int>::max())
...
#if FMT_USE_INT128
template <>
struct std::formatter<__int128_t> : std::formatter<long long> {
