...
TEST(LocaleTest, Format) {
  ...
  EXPECT_EQ("4,2,9,4,9,6,7,2,9,5", fmt::format(small_grouping_loc, "{:L}", max_value<uint32_t>()));
}
...
TEST(LocaleTest, WFormat) {
  ...
  std::locale special_grouping_loc(std::locale(), new special_grouping<wchar_t>());
  EXPECT_EQ(L"1,23,45,678", fmt::format(special_grouping_loc, L"{:L}", 12345678));

  std::locale small_grouping_loc(std::locale(), new small_grouping<wchar_t>());
  EXPECT_EQ(L"4,2,9,4,9,6,7,2,9,5", fmt::format(small_grouping_loc, L"{:L}", max_value<uint32_t>()));
}
...
TEST(LocaleTest, DoubleFormatter) {
  ...
  f.parse(parse_ctx);
  char buf[10] = {};
  fmt::basic_format_context<char*, char> format_ctx(buf, {}, fmt::detail::locale_ref(loc));
...
struct formatter<std::complex<double>, charT> {
 ...
 public:
  ...
  typename FormatContext::iterator format(const std::complex<double>& c, FormatContext& ctx) {
    ...
    auto real = fmt::format(ctx.locale().template get<std::locale>(), "{:" + specs + "}", c.real());
    auto imag = fmt::format(ctx.locale().template get<std::locale>(), "{:" + specs + "}", c.imag());
    auto fill_align_width = std::string();
    if (specs_.width > 0) fill_align_width = fmt::format(">{}", specs_.width);
    return format_to(ctx.out(), "{:" + fill_align_width + "}",
