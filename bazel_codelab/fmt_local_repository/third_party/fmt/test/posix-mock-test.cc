...
TEST(UtilTest, GetPageSize) {
  ...
  sysconf_error = true;
  EXPECT_SYSTEM_ERROR(fmt::getpagesize(), EINVAL, "cannot get memory page size");
  sysconf_error = false;
#endif
}
...
TEST(FileTest, OpenRetry) {
  ...
  EXPECT_RETRY(f.reset(new file("temp", file::RDONLY)), open, "cannot open file temp");
#ifndef _WIN32
  char c = 0;
  f->read(&c, 1);
#endif
...
TEST(FileTest, Size) {
  ...
  fmt::detail::format_windows_error(message, ERROR_ACCESS_DENIED, "cannot get file size");
  fstat_sim = ERROR;
...
TEST(FileTest, ReadRetry) {
  ...
  char buffer[SIZE];
  size_t count = 0;
  EXPECT_RETRY(count = read_end.read(buffer, SIZE), read, "cannot read from file");
...
TEST(FileTest, WriteRetry) {
  ...
  enum { SIZE = 4 };
  size_t count = 0;
  EXPECT_RETRY(count = write_end.write("test", SIZE), write, "cannot write to file");
  write_end.close();
#ifndef _WIN32
...
TEST(FileTest, DupNoRetry) {
  ...
  dup_count = 1;
  EXPECT_SYSTEM_ERROR(file::dup(stdout_fd), EINTR, fmt::format("cannot duplicate file descriptor {}", stdout_fd));
  dup_count = 0;
}
...
TEST(FileTest, Dup2Retry) {
  ...
  EXPECT_RETRY(f1.dup2(f2.descriptor()), dup2,
               fmt::format("cannot duplicate file descriptor {} to {}", f1.descriptor(), f2.descriptor()));
}
...
TEST(FileTest, PipeNoRetry) {
  file read_end, write_end;
  pipe_count = 1;
  EXPECT_SYSTEM_ERROR(file::pipe(read_end, write_end), EINTR, "cannot create pipe");
  pipe_count = 0;
}
...
TEST(FileTest, FdopenNoRetry) {
  ...
  file::pipe(read_end, write_end);
  fdopen_count = 1;
  EXPECT_SYSTEM_ERROR(read_end.fdopen("r"), EINTR, "cannot associate stream with file descriptor");
  fdopen_count = 0;
}
...
TEST(BufferedFileTest, OpenRetry) {
  ...
  EXPECT_RETRY(f.reset(new buffered_file("temp", "r")), fopen, "cannot open file temp");
#ifndef _WIN32
  char c = 0;
  if (fread(&c, 1, 1, f->get()) < 1) throw fmt::system_error(errno, "fread failed");
#endif
}
...
struct locale_mock {
  ...
  MOCK_METHOD3(newlocale, locale_type(int category_mask, const char* locale, locale_type base));
  MOCK_METHOD1(freelocale, void(locale_type locale));

  MOCK_METHOD3(strtod_l, double(const char* nptr, char** endptr, locale_type locale));
...
void _free_locale(_locale_t locale) { locale_mock::instance->freelocale(locale); }
...
double strtod_l(const char* nptr, char** endptr, locale_type locale) FMT_LOCALE_THROW {
  return locale_mock::instance->strtod_l(nptr, endptr, locale);
}
...
TEST(LocaleTest, Locale) {
  ...
  EXPECT_CALL(mock, newlocale(LC_NUMERIC_MASK, StrEq("C"), nullptr)).WillOnce(Return(impl));
...
TEST(LocaleTest, Strtod) {
  ScopedMock<locale_mock> mock;
  EXPECT_CALL(mock, newlocale(_, _, _)).WillOnce(Return(reinterpret_cast<locale_type>(42)));
  EXPECT_CALL(mock, freelocale(_));
  fmt::locale loc;
  const char* str = "4.2";
  char end = 'x';
  EXPECT_CALL(mock, strtod_l(str, _, loc.get())).WillOnce(testing::DoAll(testing::SetArgPointee<1>(&end), Return(777)));
