...
TEST(StreamingAssertionsTest, EXPECT_WRITE) {
  ...
  EXPECT_NONFATAL_FAILURE(EXPECT_WRITE(stdout, std::printf("test"), "other") << "expected failure", "expected failure");
}
#endif  // FMT_USE_FCNTL
...
TEST(AssertionSyntaxTest, ExceptionAssertionBehavesLikeSingleStatement) {
  if (::testing::internal::AlwaysFalse()) EXPECT_THROW_MSG(do_nothing(), std::exception, "");
...
TEST(AssertionSyntaxTest, SystemErrorAssertionBehavesLikeSingleStatement) {
  if (::testing::internal::AlwaysFalse()) EXPECT_SYSTEM_ERROR(do_nothing(), EDOM, "");
...
TEST(AssertionSyntaxTest, WriteAssertionBehavesLikeSingleStatement) {
  if (::testing::internal::AlwaysFalse()) EXPECT_WRITE(stdout, std::printf("x"), "x");
...
TEST(ExpectTest, EXPECT_SYSTEM_ERROR) {
  ...
  EXPECT_NONFATAL_FAILURE(EXPECT_SYSTEM_ERROR(throw_system_error(), EDOM, "other"),
                          fmt::format("throw_system_error() throws an exception with a different message.\n"
                                      "Expected: {}\n"
                                      "  Actual: {}",
                                      format_system_error(EDOM, "other"), format_system_error(EDOM, "test")));
}
...
TEST(OutputRedirectTest, FlushErrorInCtor) {
  ...
  EXPECT_SYSTEM_ERROR_NOASSERT(redir.reset(new OutputRedirect(f.get())), EBADF, "cannot flush stream");
  redir.reset(nullptr);
