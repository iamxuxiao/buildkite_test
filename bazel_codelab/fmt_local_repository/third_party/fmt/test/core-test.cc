...
TEST(BufferTest, Indestructible) {
  static_assert(!std::is_destructible<fmt::detail::buffer<int>>(), "buffer's destructor is protected");
}
...
struct mock_buffer final : buffer<T> {
  ...
  mock_buffer(T *data = nullptr, size_t buf_capacity = 0) {
    ...
    ON_CALL(*this, do_grow(_)).WillByDefault(Invoke([](size_t capacity) { return capacity; }));
  }
};
...
TEST(ArgTest, MakeValueWithCustomContext) {
  auto t = test_struct();
  fmt::detail::value<custom_context> arg(fmt::detail::arg_mapper<custom_context>().map(t));
...
struct mock_visitor {
  ...
  mock_visitor() { ON_CALL(*this, visit(_)).WillByDefault(Return(test_result())); }
...
#define CHECK_ARG_(Char, expected, value)                                                       \
  {                                                                                             \
    ...
    fmt::visit_format_arg(visitor, make_arg<fmt::basic_format_context<iterator, Char>>(value)); \
  }
...
using types = ::testing::Types<bool, signed char, unsigned char, signed, unsigned short, int, unsigned, long,
...
struct check_custom {
  test_result operator()(fmt::basic_format_arg<fmt::format_context>::handle h) const {
...
struct formatter<copy_throwable> {
  auto parse(format_parse_context &ctx) const -> decltype(ctx.begin()) { return ctx.begin(); }
  auto format(copy_throwable, format_context &ctx) -> decltype(ctx.out()) { return ctx.out(); }
};
FMT_END_NAMESPACE
...
TEST(StringViewTest, ValueType) { static_assert(std::is_same<string_view::value_type, char>::value, ""); }
...
struct formatter<enabled_formatter> {
  ...
  auto format(enabled_formatter, format_context &ctx) -> decltype(ctx.out()) { return ctx.out(); }
};
FMT_END_NAMESPACE
...
struct formatter<convertible_to_c_string> {
  FMT_CONSTEXPR auto parse(format_parse_context &ctx) -> decltype(ctx.begin()) { return ctx.begin(); }
  auto format(convertible_to_c_string, format_context &ctx) -> decltype(ctx.out()) {
...
template <typename Char>
inline fmt::basic_string_view<Char> to_string_view(const my_string<Char> &s) FMT_NOEXCEPT {
...
TEST(CoreTest, ToStringViewForeignStrings) {
  ...
  fmt::detail::type type = fmt::detail::mapped_type_constant<my_string<char>, fmt::format_context>::value;
...
TEST(CoreTest, DisabledRValueConversion) { EXPECT_EQ("foo", fmt::format("{}", disabled_rvalue_conversion())); }
